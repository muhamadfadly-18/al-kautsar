...
const contentEndpoints: Record<ContentSectionKey, string[]> = {
  banner: ["/api/content/banner", "/api/banners"],
  promotions: ["/api/content/promotions", "/api/promotions"],
  gallery: ["/api/content/gallery", "/api/gallery"],
  activities: ["/api/content/activities", "/api/activities"],
  achievements: ["/api/content/achievements", "/api/achievements"],
  about: ["/api/content/about", "/api/about"],
  extracurriculars: ["/api/content/extracurriculars", "/api/extracurriculars"],
};
...
const getFirstAvailable = async <T>(
  endpoints: string[],
...
export const getHomeContent = async (): Promise<HomeContent> => {
  const [banner, promotions, gallery, activities, achievements, about, extracurriculars] =
    await Promise.all([
      getFirstAvailable(contentEndpoints.banner, normalizeBanner, defaultHomeContent.banner),
      getFirstAvailable(contentEndpoints.promotions, normalizePromotions, defaultHomeContent.promotions),
      getFirstAvailable(contentEndpoints.gallery, normalizeGallery, defaultHomeContent.gallery),
      getFirstAvailable(contentEndpoints.activities, normalizeActivities, defaultHomeContent.activities),
      getFirstAvailable(contentEndpoints.achievements, normalizeAchievements, defaultHomeContent.achievements),
      getFirstAvailable(contentEndpoints.about, normalizeAbout, defaultHomeContent.about),
      getFirstAvailable(
...
export const saveContentSection = async <K extends ContentSectionKey>(
  ...
  const primaryEndpoint = contentEndpoints[section][0];

  try {
    const response = await api.put(primaryEndpoint, payload);
    return unwrapData<HomeContent[K]>(response.data);
  } catch (error) {
    if (error instanceof AxiosError && error.response?.status === 404) {
      const response = await api.post(primaryEndpoint, payload);
      return unwrapData<HomeContent[K]>(response.data);
    }
