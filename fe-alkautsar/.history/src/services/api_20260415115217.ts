...
const pickString = (value: unknown, fallback = "") => String(value ?? fallback).trim();
...
const normalizeStudentLocationOption = (
  ...
  return {
    ...
    name: pickString(raw.name ?? raw.title ?? raw.label ?? `${type} ${index + 1}`),
    type,
  };
};
...
const normalizeStudentClassOption = (
  ...
  return {
    ...
    name: pickString(raw.name ?? raw.title ?? raw.label ?? `Kelas ${index + 1}`),
  };
};
...
const normalizeStudentMeta = (value: unknown): StudentMeta => {
  ...
  const centers = Array.isArray(raw.centers)
    ? raw.centers.map((item, index) => normalizeStudentLocationOption(item, "Pusat", index))
    : [];
  const branches = Array.isArray(raw.branches)
    ? raw.branches.map((item, index) => normalizeStudentLocationOption(item, "Cabang", index))
    : [];
...
const normalizeStudent = (value: unknown, index: number): StudentRecord => {
  const raw = isRecord(value) ? value : {};
  const classRecord = getNestedRecord(raw, "class") ?? getNestedRecord(raw, "kelas");
  const centerRecord = getNestedRecord(raw, "center");
  const branchRecord = getNestedRecord(raw, "branch");

  const classId = pickString(raw.classId ?? raw.class_id ?? classRecord?.id ?? classRecord?._id);
  ...
  const centerId = pickString(raw.centerId ?? raw.center_id ?? centerRecord?.id ?? centerRecord?._id);
  const centerName = pickString(raw.centerName ?? raw.center_name ?? centerRecord?.name ?? centerRecord?.title);
  const branchId = pickString(raw.branchId ?? raw.branch_id ?? branchRecord?.id ?? branchRecord?._id);
  const branchName = pickString(raw.branchName ?? raw.branch_name ?? branchRecord?.name ?? branchRecord?.title);
  const hasBranch = Boolean(branchId || branchName);

  return {
    id: pickString(raw.id ?? raw._id ?? `student-${index + 1}`),
    nis: pickString(raw.nis ?? raw.nisn ?? raw.student_number),
    name: pickString(raw.name ?? raw.fullName ?? raw.nama),
    gender: normalizeStudentGender(raw.gender ?? raw.jenisKelamin ?? raw.jenis_kelamin),
    classId: classId || undefined,
    className,
    locationId: (hasBranch ? branchId : centerId) || undefined,
    locationName: hasBranch ? branchName : centerName,
    locationType: hasBranch
      ? "Cabang"
      : normalizeStudentLocationType(raw.locationType ?? raw.location_type ?? raw.type),
    ...
    phone: pickString(raw.phone ?? raw.phoneNumber ?? raw.noHp ?? raw.phone_number),
    address: pickString(raw.address ?? raw.alamat),
  };
};
...
const toStudentPayload = (student: StudentFormState) => {
  ...
  if (student.classId) {
    ...
    payload.class_id = student.classId;
  }

  if (student.locationType === "Cabang" && student.locationId) {
    payload.branchId = student.locationId;
    payload.branch_id = student.locationId;
  }

  if (student.locationType === "Pusat" && student.locationId) {
    payload.centerId = student.locationId;
    payload.center_id = student.locationId;
  }
...
export const updateStudent = async (
  ...
  const response = await api.put(`${studentsEndpoint}/${id}`, toStudentPayload(payload));
  return normalizeStudent(unwrapData<unknown>(response.data), 0);
};
...
export const importStudentsExcel = async (file: File): Promise<StudentRecord[]> => {
  const formData = new FormData();
  formData.append("file", file);

  const response = await api.post(`${studentsEndpoint}/import/excel`, formData, {
    headers: {
