...
export const getKegiatan = async (): Promise<KegiatanItem[]> => {
  const res = await api.get("/api/kegiatan");
  return res.data;
};

export const createKegiatan = async (payload: KegiatanItem) => {
  const res = await api.post("/api/kegiatan", payload);
  return res.data;
};
