...
const normalizeDDD = (value: unknown): DDDContent => {
  const raw = Array.isArray(value) ? value[0] : value;

  if (!isRecord(raw)) {
    return defaultHomeContent.DDD;
  }

  return {
    title: String(raw.title ?? defaultHomeContent.DDD.title),
    text: String(raw.text ?? raw.description ?? defaultHomeContent.DDD.text),
    image: withAssetBase(
      String(raw.image ?? raw.imageUrl ?? defaultHomeContent.DDD.image),
    ),
    primaryLabel: String(
      raw.primaryLabel ??
        raw.primary_button_label ??
        defaultHomeContent.DDD.primaryLabel,
    ),
    primaryHref: String(
      raw.primaryHref ??
        raw.primary_button_href ??
        defaultHomeContent.DDD.primaryHref,
    ),
    secondaryLabel: String(
      raw.secondaryLabel ??
        raw.secondary_button_label ??
        defaultHomeContent.DDD.secondaryLabel,
    ),
    secondaryHref: String(
      raw.secondaryHref ??
        raw.secondary_button_href ??
        defaultHomeContent.DDD.secondaryHref,
    ),
  };
};
...
export const createContentItem = async <K extends ContentSectionKey>(
  section: K,
  payload: any
) => {
...
export const updateContentItem = async <K extends ContentSectionKey>(
  section: K,
  id: string,
  payload: any
) => {
...
export const deleteContentItem = async <K extends ContentSectionKey>(
  section: K,
  id: string
) => {
  const endpoint = contentEndpoints[section];
