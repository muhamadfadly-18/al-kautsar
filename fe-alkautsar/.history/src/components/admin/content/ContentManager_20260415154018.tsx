...
const ContentManager = () => {
  ...
  await Swal.fire({
    title: index === undefined ? "Tambah Grup Ekstrakurikuler" : `Edit Grup #${index + 1}`,
    html: `
      <div style="text-align: left; font-family: 'Segoe UI', Roboto, sans-serif; color: #1f2937;">
        ...
        <div style="display: grid; grid-template-columns: 1.5fr 1fr; gap: 24px; margin-bottom: 24px;">
          ...
          <div style="display: flex; flex-direction: column; gap: 16px;">
            ${modalInput(
              ...
              "Nama Grup / Kategori",
              initialValue.title,
              "Contoh: Olahraga & Kesehatan"
            )}
            
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px;">
              <div style="display: flex; flex-direction: column; gap: 4px;">
                <label style="font-weight: 600; font-size: 13px; color: #4b5563;">Icon Bootstrap</label>
                <div style="display: flex; gap: 8px;">
                  <input id="ekskul-icon" type="text" value="${initialValue.icon}" 
                    placeholder="bi-stars"
                    style="flex: 1; padding: 10px; border: 1px solid #e2e8f0; border-radius: 10px; font-size: 14px; outline: none;" />
                  <div id="icon-preview" style="width: 42px; height: 42px; background: #f1f5f9; border-radius: 10px; display: flex; align-items: center; justify-content: center; font-size: 20px; color: #6366f1; border: 1px solid #e2e8f0;">
                    <i class="${initialValue.icon}"></i>
                  </div>
                </div>
              </div>
          ...
          <div style="background: #f8fafc; padding: 16px; border-radius: 16px; border: 1px solid #e2e8f0; display: flex; flex-direction: column; justify-content: center;">
             <div style="font-size: 12px; color: #64748b; line-height: 1.5;">
               <strong style="color: #475569; display: block; margin-bottom: 4px;">Tips Konfigurasi:</strong>
               • Gunakan nama class <a href="https://icons.getbootstrap.com/" target="_blank" style="color: #6366f1; text-decoration: none;">Bootstrap Icons</a>.<br>
               • Gradient mendukung format CSS standard.<br>
               • Daftar item di bawah akan muncul sebagai list bullet.
             </div>
          </div>
        </div>

        <div style="background: #ffffff; padding: 20px; border-radius: 16px; border: 1px solid #f1f5f9; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.05);">
           <label style="display: block; font-weight: 700; font-size: 13px; color: #1e2937; margin-bottom: 8px; text-transform: uppercase; letter-spacing: 0.5px;">Daftar Ekstrakurikuler (Satu baris satu item)</label>
           <textarea id="ekskul-items" style="
            width: 100%; height: 150px; padding: 15px; border: 1px solid #e2e8f0; border-radius: 12px;
            font-family: 'Fira Code', monospace; font-size: 14px; line-height: 1.6; resize: none;
            outline: none; transition: border-color 0.2s; box-sizing: border-box;
           " placeholder="Pramuka&#10;PMR&#10;Paskibra" onfocus="this.style.borderColor='#6366f1'" onblur="this.style.borderColor='#e2e8f0'">${initialValue.items.join("\n")}</textarea>
        </div>

        <div style="margin-top: 32px; display: flex; justify-content: flex-end; gap: 12px; border-top: 1px solid #f1f5f9; padding-top: 20px;">
          <button id="btn-cancel-ekskul" style="
            background: #ffffff; color: #64748b; padding: 12px 24px; border: 1px solid #e2e8f0;
            border-radius: 12px; font-weight: 600; font-size: 14px; cursor: pointer;">
            Batal
          </button>
          <button id="btn-save-ekskul" style="
            background: linear-gradient(135deg, #6366f1, #4f46e5); color: white; padding: 12px 32px;
            border: none; border-radius: 12px; font-weight: 600; font-size: 14px; cursor: pointer;
    ...
    showCancelButton: false,
    didOpen: () => {
      const btnSave = document.getElementById("btn-save-ekskul");
      const btnCancel = document.getElementById("btn-cancel-ekskul");
      const iconInput = document.getElementById("ekskul-icon") as HTMLInputElement;
      ...
      btnSave?.addEventListener("click", async () => {
        ...
        const itemsRaw = (document.getElementById("ekskul-items") as HTMLTextAreaElement).value;
        ...
        setContent((prev) => ({
          ...prev,
          extracurriculars: index === undefined
            ? [...prev.extracurriculars, nextItem]
            : prev.extracurriculars.map((item, i) => i === index ? nextItem : item),
        }));
