...
const modalHtml = (fields: string) => `<div class="student-modal-form"><div class="student-modal-grid">${fields}</div></div>`;

const modalInput = (id: string, label: string, value: string, placeholder = "", full = false) => `
...
const modalTextarea = (id: string, label: string, value: string, placeholder = "", full = true) => `
  <label class="student-modal-field ${full ? "student-modal-field--full" : ""}">
    <span>${label}</span>
...
const modalSummary = (title: string, description: string, image?: string, accent?: string) => (
...
const ContentManager = () => {
  ...
  const saveSection = async (section: ContentSection) => {
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : "Gagal menyimpan data ke API.";
  ...
  const openBannerModal = async () => {
    ...
    const result = await Swal.fire<BannerContent>({
      ...
      html: modalHtml(
        [
          modalInput("banner-title", "Judul Banner", banner.title, "Judul banner"),
          modalInput("banner-image", "URL Gambar Banner", banner.image, "https://..."),
          modalTextarea("banner-text", "Deskripsi Banner", banner.text, "Isi banner"),
          modalInput("banner-primary-label", "Tombol 1", banner.primaryLabel, "Label tombol"),
          modalInput("banner-primary-href", "Link Tombol 1", banner.primaryHref, "#tentang"),
          modalInput("banner-secondary-label", "Tombol 2", banner.secondaryLabel, "Label tombol"),
          modalInput("banner-secondary-href", "Link Tombol 2", banner.secondaryHref, "#promosi"),
        ].join(""),
      ),
      width: 900,
      ...
      buttonsStyling: false,
      preConfirm: () => {
        const nextBanner: BannerContent = {
    ...
    setContent((prev) => ({ ...prev, banner: result.value! }));
    if (!result.isConfirmed || !result.value) return;

const newData = result.value!;

setContent((prev) => {
  const updated = { ...prev, banner: newData };

  // 🔥 langsung simpan ke API
  saveContentSection("banner", updated.banner);
    ...
    const result = await Swal.fire<HomeContent["about"]>({
      ...
      html: modalHtml(
        [
          ...
          modalInput("about-highlight", "Highlight", about.highlight, "Pendidikan Islami"),
          modalInput("about-image", "URL Gambar", about.image, "https://..."),
          modalTextarea("about-description", "Deskripsi", about.description, "Deskripsi about"),
          modalInput("about-vision-title", "Judul Visi", about.visionTitle, "Judul visi"),
          modalTextarea("about-vision-text", "Isi Visi", about.visionText, "Isi visi"),
          modalInput("about-motto-title", "Judul Motto", about.mottoTitle, "Judul motto"),
          modalTextarea("about-motto-items", "Item Motto", about.mottoItems.join("\n"), "Satu baris satu item"),
          modalInput("about-facility-title", "Judul Fasilitas", about.facilityTitle, "Judul fasilitas"),
          modalTextarea("about-facilities", "Daftar Fasilitas", about.facilities.join("\n"), "Satu baris satu item"),
        ].join(""),
      ),
      width: 900,
      showCancelButton: true,
  ...
  const openPromoModal = async (initialValue: PromoItem, index?: number) => {
    const result = await Swal.fire<PromoItem>({
      title: index === undefined ? "Tambah Iklan / Promo" : `Edit Iklan #${index + 1}`,
      html: modalHtml(
        [
          modalInput("promo-title", "Judul", initialValue.title, "Judul promo"),
          modalInput("promo-tag", "Tag", initialValue.tag, "Tag promo"),
          modalInput("promo-accent", "Warna Accent", initialValue.accent, "#0f766e"),
          modalInput("promo-image", "URL Gambar", initialValue.image, "https://..."),
          modalTextarea("promo-text", "Deskripsi", initialValue.text, "Deskripsi promo"),
          modalInput("promo-button-text", "Teks Tombol Utama", initialValue.buttonText, "Lihat Detail"),
          modalInput("promo-detail-text", "Teks Tombol Detail", initialValue.detailText, "Detail Info"),
        ].join(""),
      ),
      width: 900,
      showCancelButton: true,
      ...
      preConfirm: () => {
        const nextPromo: PromoItem = {
          ...
          accent: fieldValue("promo-accent"),
          image: fieldValue("promo-image"),
        ...
        if (!nextPromo.title || !nextPromo.text) {
          Swal.showValidationMessage("Judul dan deskripsi promo wajib diisi.");
          return;
        }
    ...
    setContent((prev) => ({
      ...
      promotions:
        index === undefined
          ? [...prev.promotions, result.value!]
          : prev.promotions.map((item, itemIndex) => (itemIndex === index ? result.value! : item)),
    }));
  };
  ...
  const openGalleryModal = async (initialValue: GalleryItem, index?: number) => {
    const result = await Swal.fire<GalleryItem>({
      ...
      html: modalHtml(
        [
          modalInput("gallery-title", "Judul", initialValue.title, "Judul foto"),
          modalInput("gallery-category", "Kategori", initialValue.category, "Kategori"),
          modalInput("gallery-image", "URL Gambar", initialValue.image, "https://..."),
          modalTextarea("gallery-caption", "Caption", initialValue.caption, "Caption foto"),
        ].join(""),
      ),
      width: 900,
      showCancelButton: true,
    ...
    setContent((prev) => ({
      ...
      gallery:
        index === undefined
          ...
          : prev.gallery.map((item, itemIndex) => (itemIndex === index ? result.value! : item)),
    }));
  };

  const openActivityModal = async (initialValue: ActivityItem, index?: number) => {
    const result = await Swal.fire<ActivityItem>({
      title: index === undefined ? "Tambah Kegiatan" : `Edit Kegiatan #${index + 1}`,
      html: modalHtml(
        [
      ...
      preConfirm: () => {
        ...
        if (!nextItem.title || !nextItem.desc) {
          Swal.showValidationMessage("Judul dan deskripsi kegiatan wajib diisi.");
          return;
        }
    ...
    setContent((prev) => ({
      ...
      activities:
        index === undefined
          ...
          : prev.activities.map((item, itemIndex) => (itemIndex === index ? result.value! : item)),
    }));
  };

  const openAchievementModal = async (initialValue: AchievementItem, index?: number) => {
    const result = await Swal.fire<AchievementItem>({
      ...
      preConfirm: () => {
        ...
        if (!nextItem.title || !nextItem.desc) {
          Swal.showValidationMessage("Judul dan deskripsi prestasi wajib diisi.");
          return;
        }
    ...
    setContent((prev) => ({
      ...
      achievements:
        index === undefined
          ...
          : prev.achievements.map((item, itemIndex) => (itemIndex === index ? result.value! : item)),
    }));
  };
  ...
  const openExtracurricularModal = async (
    ...
    const result = await Swal.fire<ExtracurricularGroup>({
      title: index === undefined ? "Tambah Ekstrakurikuler" : `Edit Grup #${index + 1}`,
      html: modalHtml(
        [
          modalInput("ekskul-title", "Nama Grup", initialValue.title, "Nama grup"),
          modalInput("ekskul-icon", "Icon Bootstrap", initialValue.icon, "bi-stars"),
          modalInput("ekskul-gradient", "Gradient CSS", initialValue.gradient, "linear-gradient(...)"),
          modalTextarea("ekskul-items", "Daftar Item", initialValue.items.join("\n"), "Satu baris satu item"),
        ].join(""),
      ),
      width: 900,
      showCancelButton: true,
    ...
    setContent((prev) => ({
      ...
      extracurriculars:
        index === undefined
          ...
          : prev.extracurriculars.map((item, itemIndex) => (itemIndex === index ? result.value! : item)),
    }));
  };

  const confirmRemove = async (
    section: "promotions" | "gallery" | "activities" | "achievements" | "extracurriculars",
    index: number,
    label: string,
  ) => {
  ...
  const renderBannerSection = () => (
    <>
      <div className="content-editor__topbar">
        ...
        <button type="button" className="admin-button" onClick={openBannerModal}>
          Edit Banner
        </button>
      </div>
      {modalSummary(content.banner.title, content.banner.text, content.banner.image)}
    </>
  );
  ...
  const renderAboutSection = () => (
    <>
      ...
      {modalSummary(content.about.title, content.about.description, content.about.image)}
    </>
  );

  const renderListSection = <
    T extends PromoItem | GalleryItem | ActivityItem | AchievementItem | ExtracurricularGroup,
  >(
    title: string,
    addLabel: string,
    items: T[],
          ...
          <article key={item.id} className="content-item-card">
            <div className="content-item-card__header">
              ...
              <div className="student-table__actions">
                <button type="button" className="content-tabs__item" onClick={() => onEdit(item, index)}>
                  Edit
                </button>
                <button
                  type="button"
  ...
  const renderEditor = () => {
    ...
    if (activeTab === "promotions") {
      return renderListSection(
        ...
        (item, index) => void confirmRemove("promotions", index, item.title || `Iklan #${index + 1}`),
    ...
    if (activeTab === "gallery") {
      return renderListSection(
        ...
        (item, index) => void confirmRemove("gallery", index, item.title || `Foto #${index + 1}`),
    ...
    if (activeTab === "activities") {
      return renderListSection(
        ...
        (item, index) => void confirmRemove("activities", index, item.title || `Kegiatan #${index + 1}`),
    ...
    if (activeTab === "achievements") {
      return renderListSection(
        ...
        (item, index) => void confirmRemove("achievements", index, item.title || `Prestasi #${index + 1}`),
    ...
    return renderListSection(
      ...
      (item, index) => void confirmRemove("extracurriculars", index, item.title || `Grup #${index + 1}`),
  ...
  const activeLabel = contentTabs.find((item) => item.key === activeTab)?.label ?? activeTab;
    ...
    <section className="content-manager">
      <div className="content-manager__header">
        <div>
          ...
          <p>
            Semua aksi tambah, edit, hapus, dan simpan sekarang dibuat lebih ringkas
            dengan popup SweetAlert2 seperti modul data siswa.
          </p>
        </div>
        ...
        <button
          ...
          disabled={savingSection === activeTab || loading}
        >
          {savingSection === activeTab ? "Menyimpan..." : `Simpan ${activeLabel}`}
        </button>
      </div>
      ...
      {statusMessage ? <div className="content-status content-status--success">{statusMessage}</div> : null}
      {statusError ? <div className="content-status content-status--error">{statusError}</div> : null}
        ...
        <>
          ...
          <div className="content-savebar">
            <div>
              ...
              <p>Setelah edit selesai, kirim data section ini ke backend API.</p>
            </div>
