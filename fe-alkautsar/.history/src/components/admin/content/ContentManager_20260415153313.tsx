...
const ContentManager = () => {
  ...
  await Swal.fire({
    ...
    didOpen: () => {
      ...
      // --- IMAGE PREVIEW LOGIC ---
      const input = document.getElementById("promo-image-file") as HTMLInputElement;
      ...
      // --- BUTTON LOGIC ---
      document.getElementById("btn-cancel-modal")?.addEventListener("click", () => Swal.close());
      ...
      btnSave?.addEventListener("click", async () => {
        ...
        const nextPromo: PromoItem = {
          ...(index === undefined ? { id: crypto.randomUUID() } : initialValue),
          title,
        ...
        setContent((prev) => ({
          ...prev,
          promotions: index === undefined
            ? [...prev.promotions, nextPromo]
            : prev.promotions.map((item, i) => i === index ? nextPromo : item),
        }));
