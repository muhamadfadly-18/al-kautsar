...
const ContentManager = () => {
 ...
 const openActivityModal = async (
  ...
  await Swal.fire({
    title: index === undefined ? "Tambah Kegiatan Baru" : `Edit Kegiatan #${index + 1}`,
    html: `
      <div style="text-align: left; font-family: 'Segoe UI', Roboto, sans-serif; color: #1f2937;">
    ...
    didOpen: () => {
      ...
      const fileInput = document.getElementById("activity-image-file") as HTMLInputElement;
      ...
      btnSave?.addEventListener("click", async () => {
        ...
        try {
          ...
          setContent((prev) => ({
            ...prev,
            activities: index === undefined
              ? [...prev.activities, nextItem]
              : prev.activities.map((item, i) => i === index ? nextItem : item),
          }));
