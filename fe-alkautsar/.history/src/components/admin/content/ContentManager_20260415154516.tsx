...
const ContentManager = () => {
 ...
 const openAboutModal = async () => {
  ...
  await Swal.fire({
    ...
    didOpen: () => {
      const fileInput = document.getElementById("about-image-file") as HTMLInputElement;
      ...
      // Hover Effect
      uploadBox?.addEventListener("mouseover", () => { uploadBox.style.borderColor = "#0284c7"; uploadBox.style.background = "#f0f9ff"; });
      uploadBox?.addEventListener("mouseout", () => { uploadBox.style.borderColor = "#cbd5e1"; uploadBox.style.background = "#f8fafc"; });
      ...
      fileInput?.addEventListener("change", () => {
        ...
        if (file) {
          const reader = new FileReader();
          reader.onload = () => { if (preview) preview.innerHTML = `<img src="${reader.result}" style="width:100%;height:100%;object-fit:cover;" />`; };
          reader.readAsDataURL(file);
        }
      });
      ...
      document.getElementById("btn-save")?.addEventListener("click", async () => {
        ...
        if (!title || !description) return Swal.showValidationMessage("Judul dan Deskripsi wajib diisi");
        ...
        const nextAbout = {
          ...
          mottoItems: (document.getElementById("about-motto-items") as HTMLTextAreaElement).value.split("\n").filter(i => i.trim()),
          facilityTitle: fieldValue("about-facility-title"),
          facilities: (document.getElementById("about-facilities") as HTMLTextAreaElement).value.split("\n").filter(i => i.trim()),
        };
  ...
  const iconPresets = [
    'bi-stars', 'bi-trophy', 'bi-music-note-beamed', 'bi-palette', 
    'bi-dribbble', 'bi-camera-reels', 'bi-heart-pulse', 'bi-code-slash',
    'bi-megaphone', 'bi-translate', 'bi-people', 'bi-shield-check'
  ];

  await Swal.fire({
    title: index === undefined ? "Tambah Grup Ekskul" : `Edit Grup #${index + 1}`,
    html: `
      <div style="text-align: left; font-family: 'Segoe UI', Roboto, sans-serif; color: #1f2937;">
    ...
    didOpen: () => {
      ...
      document.getElementById("btn-cancel")?.addEventListener("click", () => Swal.close());
      document.getElementById("btn-save")?.addEventListener("click", async () => {
        const title = fieldValue("ekskul-title");
        if (!title) return Swal.showValidationMessage("Nama grup wajib diisi");
        ...
        const nextItem: ExtracurricularGroup = {
          ...
          items: (document.getElementById("ekskul-items") as HTMLTextAreaElement).value
