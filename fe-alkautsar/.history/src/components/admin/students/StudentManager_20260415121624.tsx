...
const genderOptions: Array<StudentGender | "Semua"> = ["Semua", "Laki-laki", "Perempuan"];
...
const createEmptyStudentForm = (locations: StudentLocationOption[]): StudentFormState => {
  ...
  return {
    ...
    classId: undefined,
    className: "",
    centerId: defaultLocation?.type === "Pusat" ? defaultLocation.id : undefined,
    centerName: defaultLocation?.type === "Pusat" ? defaultLocation.name : "",
    branchId: defaultLocation?.type === "Cabang" ? defaultLocation.id : undefined,
...
const createLocationOptionsHtml = (items: StudentLocationOption[], selectedId?: string) =>
  items
    .map(
      (item) =>
...
const StudentManager = () => {
  ...
  const [genderFilter, setGenderFilter] = useState<StudentGender | "Semua">("Semua");
  const [locationTypeFilter, setLocationTypeFilter] = useState<StudentLocationType | "Semua">("Semua");
  ...
  useEffect(() => {
    const loadStudentData = async () => {
      ...
      try {
        const [studentItems, metaItems] = await Promise.all([getStudents(), getStudentMeta()]);
        setStudents(studentItems);
        setMeta(metaItems);
      } catch (error) {
        await Swal.fire({
          title: "Gagal memuat data siswa",
          text: error instanceof Error ? error.message : "Terjadi kesalahan saat memuat data siswa.",
          icon: "error",
  ...
  const filteredStudents = useMemo(() => {
    ...
    return students.filter((student) => {
      ...
      const matchesLocationType =
        locationTypeFilter === "Semua" || student.locationType === locationTypeFilter;
      const matchesClass = classFilter === "Semua" || student.className === classFilter;

      return matchesKeyword && matchesGender && matchesLocationType && matchesClass;
    });
  }, [classFilter, genderFilter, locationTypeFilter, search, students]);

  const totalMale = students.filter((student) => student.gender === "Laki-laki").length;
  const totalFemale = students.filter((student) => student.gender === "Perempuan").length;
  const totalPusat = students.filter((student) => student.locationType === "Pusat").length;
  const totalCabang = students.filter((student) => student.locationType === "Cabang").length;
  ...
  const openStudentModal = async (
    ...
    const result = await Swal.fire<StudentFormState>({
      ...
      preConfirm: () => {
        ...
        const duplicateNis = students.some(
          (student) => student.nis === nextStudent.nis && student.id !== editingId,
        );
    ...
    try {
      ...
      setStudents((current) =>
        current.map((student) => (student.id === editingId ? updated : student)),
      );
      ...
      await Swal.fire({
        ...
        text: error instanceof Error ? error.message : "Gagal menyimpan data siswa.",
        icon: "error",
        confirmButtonText: "OK",
        customClass: { confirmButton: "student-swal-confirm" },
        buttonsStyling: false,
  ...
  const handleDelete = async (student: StudentRecord) => {
    ...
    try {
      ...
      await Swal.fire({
        ...
        text: error instanceof Error ? error.message : "Gagal menghapus data siswa.",
        icon: "error",
        confirmButtonText: "OK",
        customClass: { confirmButton: "student-swal-confirm" },
        buttonsStyling: false,
    ...
    <section className="student-manager">
      <div className="student-manager__hero">
        <div>
          ...
          <p>
            Form siswa sudah disesuaikan ke schema DB: `guardianName`, `placeOfBirth`,
            `birthDate`, `status`, `centerId`, `branchId`, dan `classId`.
          </p>
        </div>

        <div className="student-toolbar">
          <button type="button" className="admin-button" onClick={handleCreate} disabled={loading}>
            Tambah Baru
          </button>
          <button type="button" className="admin-button" onClick={handleExport} disabled={loading}>
            Export Excel
          </button>
        ...
        <article className="student-list-card">
          ...
          <div className="student-filters">
            ...
            <label className="content-field">
              ...
              <select
                value={genderFilter}
                onChange={(event) => setGenderFilter(event.target.value as StudentGender | "Semua")}
              >
            ...
            <label className="content-field">
              ...
              <select
                ...
                onChange={(event) =>
                  setLocationTypeFilter(event.target.value as StudentLocationType | "Semua")
                }
              >
