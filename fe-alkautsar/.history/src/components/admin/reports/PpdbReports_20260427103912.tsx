...
const PpdbReports = ({ onUnreadChange }: PpdbReportsProps) => {
  ...
  const loadData = async (seenAt?: string | null) => {
    setLoading(true);
    ...
    try {
      ...
      onUnreadChange?.(result.summary.baru);
    } catch (error) {
      onUnreadChange?.(0);
      await Swal.fire({
    ...
    <section className="report-manager">
      <div className="report-manager__hero">
        <div>
          ...
          <p>
            Rekap ini mengambil data dari tabel PPDB. Saat menu laporan dibuka,
            notifikasi baru langsung otomatis dianggap sudah dibaca.
          </p>
        </div>
      ...
      <article className="student-list-card report-info-card">
        <div className="admin-card__header">
          <div>
            <span className="admin-card__eyebrow">Status Laporan</span>
            <h3>Notifikasi admin berjalan otomatis</h3>
          </div>
          <strong>{loading ? "Memuat..." : "Aktif"}</strong>
        </div>
        <div className="report-info-grid">
          <div className="report-info-item">
            <strong>Pengiriman notifikasi</strong>
            <p>
              Saat form PPDB dikirim, frontend langsung mencoba mengirim
              notifikasi otomatis ke endpoint backend admin.
            </p>
          </div>
          <div className="report-info-item">
            <strong>Status badge laporan</strong>
            <p>
              Badge jumlah baru akan muncul sebelum laporan dibuka, lalu
              otomatis hilang ketika admin masuk ke menu laporan.
            </p>
          </div>
        </div>
      </article>
