...
import { getPpdbSummary } from "../services/api";
...
const AdminPage = () => {
  ...
  const handleMenuChange = (menu: AdminMenu) => {
    ...
    if (menu === "reports") {
      ...
      // ✅ tandai sudah dibaca
      localStorage.setItem("ppdb_reports_last_seen_at", now);
console.log("lastSeenAt:", localStorage.getItem("ppdb_reports_last_seen_at"));
  ...
  useEffect(() => {
    ...
    const loadPpdbBadge = async () => {
      try {
        const lastSeenAt = localStorage.getItem("ppdb_reports_last_seen_at");
        const result = await getPpdbSummary(lastSeenAt);
        if (activeMenu === "reports") {
          setReportsBadgeCount(0);
        } else {
          setReportsBadgeCount(result.summary.baru);
        }
      } catch {
        setReportsBadgeCount(0);
      }
