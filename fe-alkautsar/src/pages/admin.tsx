...
import { getLatestPpdbSeenAt, getPpdbSummary } from "../services/admin/reports";
import AdminLoginView from "../components/admin/AdminLoginView";
import AdminOverview from "../components/admin/AdminOverview";
import OrganizationManager from "../components/admin/organization";
import AdminSettingsManager from "../components/admin/settings";
    ...
    const loadPpdbBadge = async () => {
      ...
      try {
        ...
        setReportsBadgeCount(
          (activeMenu as AdminMenu) === "reports" ? 0 : result.summary.baru,
        );
      } catch {
        if (badgeRequestRef.current === requestId) {
          setReportsBadgeCount(0);
  ...
  if (session?.token) {
    ...
    return (
      <main className="admin-shell">
        <section className="admin-dashboard container-fluid px-0">
          <AdminSidebar
          ...
          <div className="admin-content">
            <div className="admin-content__inner">{renderActiveContent()}</div>
          </div>
        </section>
      </main>
    );
