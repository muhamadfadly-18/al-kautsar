...
const ContentManager = () => {
      ...
      didOpen: () => {
        ...
        btnSave?.addEventListener("click", async () => {
          ...
          try {
           ...
           await updateContentItem("banner", "main", nextBanner);
      ...
      didOpen: () => {
        ...
        document
          ...
          ?.addEventListener("click", async () => {
            ...
            await updateContentItem("about", "main", nextAbout);
            Swal.close();
          });
      },
    });
    ...
    `,
      ...
      didOpen: () => {
        ...
        btnSave?.addEventListener("click", async () => {
          ...
          if (index === undefined) {
            await createContentItem("promotions", nextPromo);
          } else {
            await updateContentItem("promotions", initialValue.id, nextPromo);
          }
          Swal.close();
        });
      },
      ...
      didOpen: () => {
        ...
        btnSave?.addEventListener("click", async () => {
          ...
          try {
            ...
            if (index === undefined) {
              await createContentItem("gallery", nextItem);
            } else {
              await updateContentItem("gallery", initialValue.id, nextItem);
            }
            Swal.close();
          } catch {
      ...
      didOpen: () => {
        ...
        btnSave?.addEventListener("click", async () => {
          ...
          try {
            ...
            if (index === undefined) {
              await createContentItem("activities", nextItem);
            } else {
              await updateContentItem("activities", initialValue.id, nextItem);
            }
            Swal.close();
          } catch {
      ...
      didOpen: () => {
        ...
        btnSave?.addEventListener("click", async () => {
          ...
          try {
            ...
            if (index === undefined) {
              await createContentItem("achievements", nextItem);
            } else {
      ...
      didOpen: () => {
        ...
        document
          ...
          ?.addEventListener("click", async () => {
            ...
            if (index === undefined) {
              await createContentItem("extracurriculars", nextItem);
            } else {
              await updateContentItem(
                "extracurriculars",
                initialValue.id,
  ...
  const confirmRemove = async (
    ...
    const result = await Swal.fire({
      ...
      didOpen: () => {
        const actions = document.querySelector(".swal2-actions") as HTMLElement;

        if (actions) {
          actions.style.display = "flex";
          actions.style.gap = "12px"; // 🔥 JARAK DI SINI
    ...
    await deleteContentItem(section, item.id);
